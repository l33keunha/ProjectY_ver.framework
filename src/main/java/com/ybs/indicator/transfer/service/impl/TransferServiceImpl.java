...
public class TransferServiceImpl extends EgovAbstractServiceImpl implements TransferService {
	...
	public void downloadTransferResultList(SearchVO sVO, HttpServletResponse res) {
		...
		String methodHeader = null;			// ▶ 수단별 헤더
		String methodStr = null;			// ▶ 수단별 컬럼
		if(sVO.getTm().equals("null") || sVO.getTm().equals("")) {
			methodHeader =  "분석지역광역도"
					+",분석지역시군"
					+",분석자료"
					+",운행일자"
					+",요일"
		...
		String numHeader = null;			// ▶ 횟수별 헤더
		String numStr = null;				// ▶ 횟수별 컬럼
		if(sVO.getTm().equals("null") || sVO.getTm().equals("")) {
			numHeader = "분석지역광역도"
					+",분석지역시군"
					+",분석자료"
