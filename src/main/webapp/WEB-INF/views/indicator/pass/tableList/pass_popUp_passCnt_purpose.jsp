						...
						<c:forEach var='j' begin='0' end='${forCnt-1}'>
								...
								<tr style="${scroll_display_none}" class= "show_${showScroll}">
									...
									<c:if test="${sVO.tm == 'allDay'}">
										...
										<td class="tg-0pky"  style="width: 70px; min-width: 70px; max-width: 70px;"><div class="lavel-2">${passResultList[j*columnCnt].opratDate}</div></td>				<!-- 날짜 -->
										<td class="tg-0pky"  style="width: 30px; min-width: 30px; max-width: 30px;"><div class="lavel-2">${passResultList[j*columnCnt].dy}</div></td>						<!-- 요일 -->
										<c:if test="${(j % forCnt) == 0}">	 
									...
									<c:if test="${sVO.tm == null}">
										...
										<c:if test="${(j % timeCnt) == 0}">	   												
											...
											<td class="tg-0pky" rowspan='${timeCnt}' 	style="width: 70px; min-width: 70px; max-width: 70px;"><div class="lavel-2">${passResultList[dateCnt*indexCnt*columnCnt].opratDate}</div></td>	<!-- 날짜 -->
											<td class="tg-0pky" rowspan='${timeCnt}' 	style="width: 30px; min-width: 30px; max-width: 30px;"><div class="lavel-2">${passResultList[dateCnt*indexCnt*columnCnt].dy}</div></td>			<!-- 요일 -->
