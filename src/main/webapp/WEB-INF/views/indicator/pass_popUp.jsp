   ...
   <!-- ▼ 코드 텍스트 매칭 -->
    ...
    <body>
      <div class="wrap">
        ...
        <div class="dateform">
          <div>
          <p>분석날짜 : ${pVO.passDateStart} ~ ${pVO.passDateEnd}</p>
				...
				<table class="tg" style="table-layout: fixed; word-wrap: break-word;">
               		<tbody>
                                ...
                                <c:forEach var='j' begin='0' end='${forCnt-1}'>
                                    <tr>
                                        ...
                                        <c:if test="${(j % dateCnt) == 0}">
                                            ...
                                            <td class="tg-0pky" rowspan='${dateCnt}' style="width: 80px; min-width: 80px; max-width: 80px;">버스</td>											<!-- 노선구분 -->
                                            <c:set var = "indexCnt" value = "${indexCnt + 1}"/>
                                        </c:if>
                                    
                                        <!-- 노선명 그룹이 시작될 때마다 실행 -->
                                        <c:if test="${(j % timeCnt) == 0}">	   	
                                            <td class="tg-0pky" rowspan='${timeCnt}' style="width: 80px; min-width: 80px; max-width: 80px;">${passResultListB[j * columnCnt].passrouteKr }</td>		<!-- 노선명 -->
                                        </c:if>
                                        ...
                                        <c:forEach var='i' begin='${st}' end='${ed-1}' >
                                            <td class="tg-0pky" style="width: 70px; min-width: 70px; max-width: 70px;">${passResultListB[i].passrtebincnt}</td>		<!-- 승차 -->
                                            <td class="tg-0pky" style="width: 70px; min-width: 70px; max-width: 70px;">${passResultListB[i].passrteboutcnt}</td>	<!-- 하차 -->
                                            <td class="tg-0pky" style="width: 70px; min-width: 70px; max-width: 70px;">${passResultListB[i].passrtebhscnt}</td>		<!-- 환승 -->
                                        
                                            <c:set var = "totalInSum" 	value = '${totalInSum + (passResultListB[i].passrtebincnt)}'/>		<!-- 승차합 계산 -->
                                            <c:set var = "totalOutSum" 	value = '${totalOutSum + (passResultListB[i].passrteboutcnt)}'/>	<!-- 하차합 계산 -->
                                            <c:set var = "totalHsSum" 	value = '${totalHsSum + (passResultListB[i].passrtebhscnt)}'/>		<!-- 환승합 계산 -->
                                        </c:forEach>
                        ...
                        <c:if test="${passTypeText == '수단통행'}">
							...
							<!-- 버스 and 지하철 일떄 -->
							<c:if test="${pVO.passTransport =='00'}">
								<c:set var = "forCnt" value = '${passResultList.size()/columnCnt/2}'/>	<!-- 컬럼갯수 에 따른 tr, td 생성 -->
							</c:if>
					    
	                    	<!-- 시간 -->
    						<c:set var = "timeCnt" value = "${pVO.passTimeEnd - pVO.passTimeStart + 1}"/>
							...
							<c:forEach var='j' begin='0' end='${forCnt-1}'>
								<tr>
									<!-- 조회한 시간에 따라 실행 -->
									<c:if test="${(j % timeCnt) == 0}">	   
										<td class="tg-0pky" rowspan='${timeCnt}' style="width: 70px; min-width: 70px; max-width: 70px;">${passResultList[j].passdate}</td>		<!-- 날짜 -->
										<td class="tg-c3ow" rowspan='${timeCnt}' style="width: 80px; min-width: 80px; max-width: 80px;">경기도</td>								<!-- 광역/도 -->
										<td class="tg-0pky" rowspan='${timeCnt}' style="width: 80px; min-width: 80px; max-width: 80px;">평택시</td>								<!-- 시/군 -->
									</c:if>
									...
									<!-- 컬럼 갯수에 걸쳐 한번만 실행 -->
									<c:if test="${pVO.passTransport =='00'}">
										<td class="tg-0pky" style="width: 30px; min-width: 30px; max-width: 30px;">${passResultList[j*columnCnt*2].passtime}</td>										<!-- 시간 -->
									</c:if>
									...
									<c:forEach var='i' begin='${st}' end='${ed-1}' >	    
										...
										<!-- 버스 and 지하철 일떄 -->
										<!-- 버스, 지하철 2번에 걸쳐 한번만 실행 -->
										<c:if test="${pVO.passTransport == '00' }">
											<td class="tg-0pky" style="width: 70px; min-width: 70px; max-width: 70px;">${passResultList[i*2].rownum}</td>											<!-- 버스-->
											<td class="tg-0pky" style="width: 70px; min-width: 70px; max-width: 70px;">${passResultList[(i*2)+1].rownum}</td>										<!-- 지하철-->
											<td class="tg-0pky" style="width: 70px; min-width: 70px; max-width: 70px;">${passResultList[(i*2)].rownum + passResultList[(i*2)+1].rownum}</td>		<!-- 소계(버스+지하철) -->	
											<c:set var = "totalSum" value = '${totalSum + (passResultList[(i*2)].rownum + passResultList[(i*2)+1].rownum)}'/>	
										</c:if>
