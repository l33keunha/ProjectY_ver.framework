...
<html>
    ...
    <!-- 코드 텍스트 매칭 -->
    <%@ include file="timeDistn_popUp_codeNameMatch.jsp" %>
	...
	<!-- 그래프 조회조건 -->
	<form id="graphForm" action="timeDistnGraph.do" target="graph" method="post">
		<input name="anal_fin" type="hidden" id="anal_finJS" value="${sVO.anal_fin}"/>
		<input name="dateStart" type="hidden" id="dateStartJS" value="${sVO.dateStart}"/>
		<input name="dateEnd" type="hidden" id="dateEndJS" value="${sVO.dateEnd}"/>
		<input name="anal_area_cd_sido" type="hidden" id="anal_area_cd_sidoJS" value="${sVO.anal_area_cd_sido}"/>
	     	...
	     	<div class="test">
		     	...
		     	<div class="content_wrap">
			     	<c:if test = "${anal_groupText == '1일 평균 통행시간/거리'}">
			     		<%@ include file="tableList/timeDistn_popUp_avg_purpose.jsp" %>
			     	</c:if> 
			     	...
			     	<c:if test = "${anal_groupText == '수단통행 평균 통행시간/거리'}">
			     		<%@ include file="tableList/timeDistn_popUp_avg_method.jsp" %>
			     	</c:if> 
			     	
			     	<c:if test = "${anal_groupText == '노선별 평균 통행시간/거리'}">
			     		<%@ include file="tableList/timeDistn_popUp_avg_route.jsp" %>
			     	</c:if> 
		     	</div>
		     </div>
		</div>
